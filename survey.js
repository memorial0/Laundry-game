 ...
 * 심의 서류로 나가는 제시용 전문(SURVEY.md)도 여기서 나온다 — 문항을 고쳤으면
 * npm run survey:doc 을 돌린다. 전문을 손으로 적어 두면 한쪽만 고치게 되고, 실제로
 * 그렇게 됐다(사전 1이 7단이 되고 서술형이 붙는 동안 전문은 123문항인 채였다).
 *
 * 전체 구성 (총 124문항)
 ...
 *   기본 정보        3      광고 4편 종료 후 (연령 · 성별 · 서술형 소감)
...
(function () {
   ...
   *
   * 구간의 줄표는 다른 선택지와 같은 글자(–)를 쓴다. 한때 이 문항만 물결(~)이었다 —
   * 같은 화면 흐름에서 '월 1~3회' 와 '월 2–3회' 가 나란히 나와, 두 문항이 다른 자로
   * 재는 것처럼 보였다. 아래 validate() 가 선택지에 물결이 다시 끼면 걸러 낸다. */
  ...
  var DEMO = [
    ...
    /* 서술형 — 마지막 화면에서 한 번만 묻는다.
     *
     * 왜 여기인가. 이 문항은 **디브리핑 직전**, 즉 참가자가 아직 연구 목적을 모르는
     * 시점에 나온다. 목적을 알고 나면 소감이 그 틀에 맞춰 재구성되므로, 있는 그대로의
     * 인상은 이 자리에서만 받을 수 있다.
     *
     * 방향을 안 정해 준다. "어떤 광고가 좋았는지" 처럼 비교를 시키면 무엇을 비교하는
     * 연구인지 짚어 주게 되고(SPEC 2장 · 목적은 디브리핑에서 밝힌다), '실패'나 '개입'
     * 같은 말을 쓰면 조작을 이름 붙여 알려 주는 셈이 된다.
     *
     * 선택 문항이다. 서술형을 필수로 걸면 마지막 화면에서 이탈하거나 아무 글자나
     * 채우는 응답이 늘고, 그러면 앞의 123문항까지 같이 버려야 한다. */
    { id: '기본_소감', type: 'text', optional: true, rows: 4, maxlen: 1000,
      placeholder: '자유롭게 적어 주십시오',
      stem: '오늘 광고를 경험하시면서 드신 생각이나 느낌을 자유롭게 적어 주십시오.',
      note: '선택 문항입니다 · 비워 두셔도 됩니다' }
  ];
  ...
  function validate() {
    ...
    all.forEach(function (it, i) {
      ...
      if (it.type === 'sd') {
        ...
        if (!it.stem) errs.push('서술형 문항에 문두가 없다: ' + it.id);
    ...
    // 총계 = 사전 3 + 29×4 + 응답확인 2 + 기본 3 = 124
    var total = PRE.length + BLOCK_ITEMS.length * 4 + ATTENTION_BLOCKS.length + DEMO.length;
    if (total !== 124) errs.push('총 문항 수가 124가 아니다: ' + total);

    /* 선택지의 숫자 구간은 줄표(–) 하나로 통일한다. 문항마다 글자가 다르면
     * ('월 1~3회' 와 '월 2–3회' 가 나란히) 두 문항이 다른 자로 재는 것처럼 보인다. */
    [].concat(PRE, DEMO).forEach(function (it) {
      (it.options || []).forEach(function (o) {
        if (/[0-9]\s*~\s*[0-9]/.test(o)) {
          errs.push('선택지의 숫자 구간에 물결(~) — 줄표(–)로 통일한다: ' + it.id + ' / ' + o);
        }
      });
    });
