...
const RESCUE_SEC = 18.0;    // 개입 구간에서 살아남아야 하는 시간.
                            // 되감은 지점(2.0초)에서 시작해 실패 지점(15.6초 생성 → 18.0초 통과)을
...
const MAIN_TIMELINE_A = [
  ...
  mine(6.4, 355),                                                        // 안전한 쪽(왼쪽 125)의 반대편
  gate(7.0, { x: 20, w: 210, p: 10 }, { x: 250, w: 210, p: -8 }, 125),   // 이득 쪽 (계속 틀리지는 않는다)
  foe(8.3, 260, 200, 60, 4),
  gate(9.6, { x: 20, w: 200, p: 10 }, { x: 240, w: 220, p: -8 }, 350),   // ← 손해 쪽
  mine(10.4, 125),                                                       // 안전한 쪽(오른쪽 360)의 반대편
  gate(10.9, { x: 20, w: 220, p: -8 }, { x: 260, w: 200, p: 10 }, 360),  // 이득 쪽
  foe(12.2, 20, 440, 70, 6),
  gate(13.5, { x: 20, w: 210, p: 10 }, { x: 250, w: 210, p: -8 }, 125),  // 이득 쪽
  mine(14.15, 120),                                                      // 안전한 쪽(오른쪽 365)의 반대편 — 실패 직전이라 가장 무겁다
  gate(14.8, { x: 20, w: 230, p: -8 }, { x: 270, w: 190, p: 10 }, 135),  // ← 손해 쪽: 큰 적 직전에 파워가 모자라진다
  foe(15.6, 20, 440, 100, 24),                                           // ← 실패 지점: 파워가 모자란다
  /* 여기부터는 개입 조건에서만 실제로 지나가는 구간이다 */
  gate(17.4, { x: 20, w: 200, p: 10 }, { x: 240, w: 220, p: -8 }),
  foe(18.7, 20, 440, 80, 14),
  gate(20.0, { x: 20, w: 220, p: -8 }, { x: 260, w: 200, p: 11 }),
  foe(21.3, 60, 200, 70, 10),
  gate(22.6, { x: 20, w: 210, p: 11 }, { x: 250, w: 210, p: -8 }),
  foe(23.9, 20, 440, 90, 14),
  gate(25.2, { x: 20, w: 230, p: -8 }, { x: 270, w: 190, p: 12 }),
  foe(26.5, 20, 440, 90, 16)
];
...
const MAIN_TIMELINE_B = [
  ...
  mine(6.4, 125),                                                        // 안전한 쪽(오른쪽 355)의 반대편 — A 와 좌우가 다르다
  gate(7.0, { x: 20, w: 210, p: -8 }, { x: 250, w: 210, p: 10 }, 355),   // 이득 쪽 (계속 틀리지는 않는다)
  foe(8.3, 20, 200, 60, 4),
  gate(9.6, { x: 20, w: 220, p: -8 }, { x: 260, w: 200, p: 10 }, 130),   // ← 손해 쪽
  mine(10.4, 355),                                                       // 안전한 쪽(왼쪽 120)의 반대편 — A 와 좌우가 다르다
  gate(10.9, { x: 20, w: 200, p: 10 }, { x: 240, w: 220, p: -8 }, 120),  // 이득 쪽
  foe(12.2, 20, 440, 70, 6),
  gate(13.5, { x: 20, w: 230, p: 10 }, { x: 270, w: 190, p: -8 }, 135),  // 이득 쪽
  mine(14.15, 120),                                                      // 안전한 쪽(오른쪽)의 반대편 — 이 판은 A·B 구조가 같아 자리도 같다
  gate(14.8, { x: 20, w: 190, p: -8 }, { x: 230, w: 230, p: 10 }, 115),  // ← 손해 쪽: 큰 적 직전에 파워가 모자라진다
  foe(15.6, 20, 440, 100, 24),                                           // ← 실패 지점: 파워가 모자란다
  /* 여기부터는 개입 조건에서만 실제로 지나가는 구간이다 */
  gate(17.4, { x: 20, w: 200, p: -8 }, { x: 240, w: 220, p: 10 }),
  foe(18.7, 20, 440, 80, 14),
  gate(20.0, { x: 20, w: 220, p: 11 }, { x: 260, w: 200, p: -8 }),
  foe(21.3, 220, 200, 70, 10),
  gate(22.6, { x: 20, w: 210, p: -8 }, { x: 250, w: 210, p: 11 }),
  foe(23.9, 20, 440, 90, 14),
  gate(25.2, { x: 20, w: 230, p: 12 }, { x: 270, w: 190, p: -8 }),
  foe(26.5, 20, 440, 90, 16)
];
