...
const MINE_COST = 8;        // 못 맞히고 지나치면 깎이는 파워.
                            // 2026-09-04 에 7 → 8 로 올렸다(난이도 상향). 이 값을 고른 이유는
                            // **시청 조건의 각본을 안 건드리기 때문**이다 — 자동 조종은 기뢰를 전부
                            // 맞히므로(파워 추적으로 확인) watch 의 파워 흐름과 실패 지점이 그대로다.
                            // START_POWER 를 내려도 난이도는 오르지만 그쪽은 watch 의 최소 파워를
                            // 같이 깎아, 3 밖에 안 남은 지금은 각본이 무너질 위험이 있다.

const RESCUE_SEC = 15.2;    // 개입 구간에서 살아남아야 하는 시간.
                            // 되감은 지점(2.0초)에서 시작해 실패 지점(13.0초 생성 → 15.2초 통과)을
                            // 2.0초 넘긴 자리에서 끝난다 — 실패를 넘겼다는 것이 보이는 최소 길이다.
                            // 아래 REWIND_FLOOR_SEC 와 한 쌍이라 따로 줄이면 실패 지점 전에 끝난다.
                            // 2026-09-04: 도입부 3개(7.0 게이트 · 8.3 적 · 9.6 게이트)를 걷어내
                            // 실패가 2.6초 당겨지면서 18.0 → 15.2 로 같이 당겼다. 이 값을 안 따라
                            // 옮기면 구출 구간이 실패 지점을 한참 넘겨서 끝나 개입형만 길어진다.
const TUTORIAL_SEC = 8;     // 조작 연습 길이
const REWIND_SEC = 16;      // 실패 지점에서 되감아 돌아가는 깊이 (되돌림 기록 상한이기도 하다).
                            // 자동 조종의 첫 오판(3.1초 생성 → 5.5초 통과)보다 앞으로 돌아가야 한다 —
                            // 이미 손해를 본 상태에서 시작하면 참가자가 만회할 수 없다.
                            // 도입부를 걷어내며 실패 지점이 두 번 당겨졌지만(8초 + 2.6초) 이 값은
                            // 그대로 두고 바닥(REWIND_FLOOR_SEC)을 내렸다 — 이유는 그쪽 주석 참고
 ...
 *
 * 왜 두 번째로 걷어낼 때는 이 값이 안 바뀌었나
 *   노출 시간을 세탁과 맞추며 도입부 2개(옛 0.5초 게이트 +7 · 옛 1.8초 적 −7)를 더 들어냈다.
 *   이 둘의 순증이 **정확히 0** 이라 시작 파워는 23 그대로다. 걷어낼 짝을 이렇게 고른 것이지
 *   우연이 아니다 — 순증이 0 이 아닌 짝을 고르면 여기를 같이 고쳐야 하고, 잘못 세면 위와
 *   같은 방식으로 각본이 무너진다. 다음에 또 줄일 일이 있으면 순증부터 계산할 것. */
const START_POWER = 23;
const REWIND_FLOOR_SEC = 2; // 아무리 되감아도 이 시각 이전으로는 가지 않는다.
                            //
                            // 왜 3 이 아니라 2 인가 — 되감은 지점의 **파워**를 맞추기 위해서다.
                            // 사건은 생성 뒤 2.4초를 이동해 배에 닿는다. 도입부 2개를 더 걷어내
                            // 실패가 18.0초에 일어나므로 16초를 되감으면 2.0초가 되는데, 바닥이 3 이면
                            // 3.0초에서 멈춘다 — 그 사이 첫 게이트(+9)가 이미 지나가 파워 32 로 시작하게 되고
                            // 구출이 그만큼 쉬워진다. 2.0초는 아직 아무 사건도 안 지난 자리라
                            // 파워가 START_POWER 그대로다. 걷어내기 전(4.6초 · 파워 23)과 같은 조건이다.
 ...
 *
 * 도입부는 두 번에 걸쳐 걷어냈고, 두 번 다 **아래 전개는 그대로 두는 방식**으로 했다.
 *   ① 6개 사건(옛 0.7~7.2초, 자동 조종이 전부 맞게 고르던 구간) — 제품 카드 8초가 붙어
 *      자극이 세탁보다 8초 길어졌을 때. 그 구간의 순증(+13)을 START_POWER 에 얹었다.
 *   ② 2개 사건(옛 0.5초 게이트 +7 · 옛 1.8초 적 −7) — 세탁과 노출 시간을 맞출 때.
 *      순증이 0 이라 START_POWER 는 그대로다.
 * 두 번 합쳐 시각이 10.6초 앞당겨졌을 뿐, 각 게이트에서의 파워도 실패 지점도 구출 난이도도
 * 예전과 같다. 남은 도입부는 **맞게 고르는 게이트 한 번**(0.5초)이고, 자동 조종이 무능해
 * 보이지 않게 하는 몫은 후반의 이득 쪽 선택 세 번(7.0 · 10.9 · 13.5초)이 맡는다.
 */
const MAIN_TIMELINE_A = [
  gate(0.5, { x: 20, w: 220, p: 9 }, { x: 260, w: 200, p: -5 }, 130),    // 이득 쪽
  foe(1.8, 20, 440, 80, 8),
  mine(2.45, 350),                                                       // 안전한 쪽(왼쪽 120)의 반대편
  gate(3.1, { x: 20, w: 200, p: 10 }, { x: 240, w: 220, p: -8 }, 350),   // ← 손해 쪽: 자동 조종의 첫 오판
  foe(4.4, 40, 180, 60, 5),
  gate(5.7, { x: 20, w: 230, p: -8 }, { x: 270, w: 190, p: 10 }, 135),   // ← 손해 쪽
  mine(6.4, 125),                                                        // 안전한 쪽(오른쪽 360)의 반대편 — 짝이던 옛 7.0 게이트를 걷어내며 좌우를 뒤집었다
  mine(7.8, 125),                                                       // 안전한 쪽(오른쪽 360)의 반대편
  gate(8.3, { x: 20, w: 220, p: -8 }, { x: 260, w: 200, p: 10 }, 360),  // 이득 쪽
  foe(9.6, 20, 440, 70, 6),
  gate(10.9, { x: 20, w: 210, p: 10 }, { x: 250, w: 210, p: -8 }, 125),  // 이득 쪽
  mine(11.55, 120),                                                      // 안전한 쪽(오른쪽 365)의 반대편 — 실패 직전이라 가장 무겁다
  gate(12.2, { x: 20, w: 230, p: -8 }, { x: 270, w: 190, p: 10 }, 135),  // ← 손해 쪽: 큰 적 직전에 파워가 모자라진다
  foe(13.0, 20, 440, 100, 24),                                           // ← 실패 지점: 파워가 모자란다
  /* 여기부터는 개입 조건에서만 실제로 지나가는 구간이다 */
  gate(14.8, { x: 20, w: 200, p: 10 }, { x: 240, w: 220, p: -8 }),
  foe(16.1, 20, 440, 80, 14),
  gate(17.4, { x: 20, w: 220, p: -8 }, { x: 260, w: 200, p: 11 }),
  foe(18.7, 60, 200, 70, 10),
  gate(20.0, { x: 20, w: 210, p: 11 }, { x: 250, w: 210, p: -8 }),
  foe(21.3, 20, 440, 90, 14),
  gate(22.6, { x: 20, w: 230, p: -8 }, { x: 270, w: 190, p: 12 }),
  foe(23.9, 20, 440, 90, 16)
];

/* ver B — A 의 평행 자극.
 *
 * 같은 것: 사건 수(게이트 12 · 적 9), 각 사건의 시각, 이득/손해 값, 적 HP,
 *          자동 조종이 오판하는 지점(3.1 · 5.7 · 9.6 · 14.8), 실패 지점(15.6초).
 ...
 *   A: 좌좌 우 좌좌 우 좌 우 좌 우 좌 우
 *   B: 우 좌좌 우우 좌좌 우우 좌 우 좌
 *  12개 중 3개(2·7·8번째 게이트)가 A 와 같은 쪽이다. 도입부 게이트 하나가 빠지며
 *  13개 중 3개에서 12개 중 3개가 됐다 — 두 제약은 그대로 지켜진다(parallel.cjs 가 검사한다).
 */
const MAIN_TIMELINE_B = [
  gate(0.5, { x: 20, w: 200, p: -5 }, { x: 240, w: 220, p: 9 }, 350),    // 이득 쪽
  foe(1.8, 20, 440, 80, 8),
  mine(2.45, 350),                                                       // 안전한 쪽(왼쪽)의 반대편 — 이 판은 A·B 구조가 같아 자리도 같다
  gate(3.1, { x: 20, w: 200, p: 10 }, { x: 240, w: 220, p: -8 }, 350),   // ← 손해 쪽: 자동 조종의 첫 오판
  foe(4.4, 260, 180, 60, 5),
  gate(5.7, { x: 20, w: 210, p: 10 }, { x: 250, w: 210, p: -8 }, 355),   // ← 손해 쪽
  mine(6.4, 355),                                                        // 안전한 쪽(왼쪽 120)의 반대편 — A 와 좌우가 다르다. 옛 7.0 게이트 제거로 뒤집었다
  mine(7.8, 355),                                                       // 안전한 쪽(왼쪽 120)의 반대편 — A 와 좌우가 다르다
  gate(8.3, { x: 20, w: 200, p: 10 }, { x: 240, w: 220, p: -8 }, 120),  // 이득 쪽
  foe(9.6, 20, 440, 70, 6),
  gate(10.9, { x: 20, w: 230, p: 10 }, { x: 270, w: 190, p: -8 }, 135),  // 이득 쪽
  mine(11.55, 120),                                                      // 안전한 쪽(오른쪽)의 반대편 — 이 판은 A·B 구조가 같아 자리도 같다
  gate(12.2, { x: 20, w: 190, p: -8 }, { x: 230, w: 230, p: 10 }, 115),  // ← 손해 쪽: 큰 적 직전에 파워가 모자라진다
  foe(13.0, 20, 440, 100, 24),                                           // ← 실패 지점: 파워가 모자란다
  /* 여기부터는 개입 조건에서만 실제로 지나가는 구간이다 */
  gate(14.8, { x: 20, w: 200, p: -8 }, { x: 240, w: 220, p: 10 }),
  foe(16.1, 20, 440, 80, 14),
  gate(17.4, { x: 20, w: 220, p: 11 }, { x: 260, w: 200, p: -8 }),
  foe(18.7, 220, 200, 70, 10),
  gate(20.0, { x: 20, w: 210, p: -8 }, { x: 250, w: 210, p: 11 }),
  foe(21.3, 20, 440, 90, 14),
  gate(22.6, { x: 20, w: 230, p: 12 }, { x: 270, w: 190, p: -8 }),
  foe(23.9, 20, 440, 90, 16)
];
 ...
 * 노출 시간(28.0초/49.7초)·난이도·A/B 평행성은 영향을 받지 않는다.
 *
 * 왜 심해인가
...
const MicrogateExperiment = () => {
  ...
  const endStimulus = useCallback((result) => {
    ...
    /* 결과 연출 뒤 제품 카드까지의 사이. 세탁 자극과 노출 시간을 맞추며 각각 0.8초·0.6초
     * 줄였다(옛 3500·2500) — 연출 자체가 아니라 그 뒤 화면이 서 있던 시간이다. */
    const delay = success ? 2700 : 1900;
    setTimeout(() => {
