(() => {
  ...
  var IS_TUTORIAL = CFG.mode === MODES.TUTORIAL;
  var IS_INTERVENE = CFG.mode === MODES.INTERVENE;
  var STIM = IS_TUTORIAL ? "game_tutorial" : "game";
  var LOG = {
    sid: CFG.sid,
    stim: STIM,
    mode: CFG.mode,
    ...
    // --- 게임 전용 ---
    RESULT: "",
    // scripted_fail | rescue_success | rescue_fail
    FAIL_REASON: null,
    // 마지막 실패 사유 문자열 (성공으로 끝나면 그대로 null)
    COLLISION_T: null,
    // 초, 자극 시작~자동재생 충돌
    POWER_END: null,
    GATES_PASSED: 0,
    VER_FALLBACK: 0
    // ver=B 를 요청했지만 B 타임라인이 없어 A로 대체됨
  };
  ...
  function secondsSince(from) {
    return from ? round2((Date.now() - from) / 1e3) : null;
  }
  var finished = false;
  function finish() {
  ...
  var STAGES = {
    PLAY: "play",
    ENDED: "ended"
    // 종료 통지 후 러너가 화면을 걷어갈 때까지의 대기 화면
  };
  var CANVAS_W = 480;
  var CANVAS_H = 720;
  var TICK_HZ = 60;
  ...
  var sec = (s) => Math.round(s * TICK_HZ);
  var RESCUE_SEC = 20.3;
  var TUTORIAL_SEC = 8;
  var REWIND_SEC = 14;
  var REWIND_FLOOR_SEC = 3;
  var REWIND_SPEED = 15;
  var gate = (t, left, right, autoTarget) => {
  ...
  var foe = (t, x, w, h, hp) => ({ type: "enemy", t: sec(t), x, w, h, hp });
  var TUTORIAL_TIMELINE = [
  ...
  var MAIN_TIMELINE_A = [
    gate(0.7, { x: 20, w: 210, p: 8 }, { x: 250, w: 210, p: -4 }, 125),
    // 이득 쪽
    foe(2, 20, 440, 60, 4),
    gate(3.3, { x: 20, w: 200, p: -4 }, { x: 240, w: 220, p: 7 }, 350),
    // 이득 쪽
    foe(4.6, 300, 160, 60, 5),
    gate(5.9, { x: 20, w: 230, p: 8 }, { x: 270, w: 190, p: -5 }, 135),
    // 이득 쪽
    foe(7.2, 20, 440, 70, 6),
    gate(8.5, { x: 20, w: 190, p: -5 }, { x: 230, w: 230, p: 7 }, 345),
    // 이득 쪽
    foe(9.8, 20, 440, 70, 7),
    gate(11.1, { x: 20, w: 220, p: 9 }, { x: 260, w: 200, p: -5 }, 130),
    // 이득 쪽
    foe(12.4, 20, 440, 80, 8),
    gate(13.7, { x: 20, w: 200, p: 10 }, { x: 240, w: 220, p: -8 }, 350),
    // ← 손해 쪽: 자동 조종의 첫 오판
    foe(15, 40, 180, 60, 5),
    gate(16.3, { x: 20, w: 230, p: -8 }, { x: 270, w: 190, p: 10 }, 135),
    // ← 손해 쪽
    gate(17.6, { x: 20, w: 210, p: 10 }, { x: 250, w: 210, p: -8 }, 125),
    // 이득 쪽 (계속 틀리지는 않는다)
    foe(18.9, 260, 200, 60, 4),
    gate(20.2, { x: 20, w: 200, p: 10 }, { x: 240, w: 220, p: -8 }, 350),
    // ← 손해 쪽
    gate(21.5, { x: 20, w: 220, p: -8 }, { x: 260, w: 200, p: 10 }, 360),
    // 이득 쪽
    foe(22.8, 20, 440, 70, 6),
    gate(24.1, { x: 20, w: 210, p: 10 }, { x: 250, w: 210, p: -8 }, 125),
    // 이득 쪽
    gate(25.4, { x: 20, w: 230, p: -8 }, { x: 270, w: 190, p: 10 }, 135),
    // ← 손해 쪽: 큰 적 직전에 파워가 모자라진다
    foe(26.2, 20, 440, 100, 24),
    // ← 실패 지점: 파워가 모자란다
    /* 여기부터는 개입 조건에서만 실제로 지나가는 구간이다 */
    gate(28, { x: 20, w: 200, p: 10 }, { x: 240, w: 220, p: -8 }),
  ...
  var TIMELINES = { A: MAIN_TIMELINE_A, B: null };
  var resolveTimeline = () => {
  ...
  var MicrogateExperiment = () => {
    const [stage, setStage] = (0, import_react.useState)(STAGES.PLAY);
    const [gamePhase, setGamePhase] = (0, import_react.useState)("none");
    const [gameResult, setGameResult] = (0, import_react.useState)("");
    const canvasRef = (0, import_react.useRef)(null);
    const reqRef = (0, import_react.useRef)(null);
    ...
    const input = (0, import_react.useRef)({ left: false, right: false, mouseX: null });
    const marks = (0, import_react.useRef)({ interveneStart: 0, firstDrag: 0 });
    const initGame = () => {
      gameState.current = {
        ship: { x: 220, y: 550, w: 40, h: 40, power: 10, isDead: false, vx: 0, vy: 0, ang: 0, av: 0 },
        gates: [],
        enemies: [],
        stars: Array.from({ length: 30 }, () => ({ x: Math.random() * 480, y: Math.random() * 720, s: 1 + Math.random() * 3 })),
        particles: [],
        rings: [],
        speed: 5,
        time: 0,
    ...
    const endStimulus = (0, import_react.useCallback)((result) => {
      ...
      if (success) {
        state.resultAnim = { type: result === "rescue_success" ? "\uAC1C\uC785 \uC131\uACF5" : "\uC5F0\uC2B5 \uC644\uB8CC", t: 1 };
        state.vignette = -2.5;
        state.flash = 1;
        state.shake = 12;
        state.ship.vy = -18;
        ...
        for (let i = 0; i < 4; i++) {
          setTimeout(() => {
            state.rings.push({ x: state.ship.x + state.ship.w / 2, y: state.ship.y + state.ship.h / 2, r: 10, life: 1, speed: 12 + i * 4, color: i % 2 === 0 ? "#ffffff" : "#00ffff" });
          }, i * 100);
        }
        ...
        state.resultAnim = { type: "FAILURE", t: 1 };
        state.vignette = 1;
        setGameResult("fail");
        setTimeout(() => {
          setGamePhase("ended");
    ...
    const stepGame = (0, import_react.useCallback)(() => {
      const state = gameState.current;
      ...
      if ((gamePhase === "autoplay_fail_watch" || gamePhase === "rewind_rescue" || gamePhase === "tutorial_play") && !state.ship.isDead && !state.isEnding) {
        if (gamePhase === "autoplay_fail_watch") {
          ...
          if (imminentEnemy && state.ship.power < imminentEnemy.hp) {
            ...
            state.slowFactor = Math.min(1, state.slowFactor + 0.1);
          }
        } else {
          state.time++;
          state.slowFactor = 1;
        ...
        checkCollisions(state, (reason) => {
          ...
          if (IS_TUTORIAL) {
            endStimulus("tutorial_fail");
            return;
          }
          if (gamePhase === "rewind_rescue") {
            endStimulus("rescue_fail");
            return;
          }
          if (!IS_INTERVENE) {
        ...
        if (state.history.length > 0 && state.time > sec(REWIND_FLOOR_SEC)) {
          ...
          state.rescueGoal = state.time + sec(RESCUE_SEC);
          marks.current.interveneStart = Date.now();
      ...
      if (state.resultAnim.type && state.resultAnim.t > 0) {
        state.resultAnim.t -= state.resultAnim.type !== "FAILURE" ? 4e-3 : 0.01;
      }
    }, [gamePhase, gameResult, endStimulus]);
    const renderGame = (0, import_react.useCallback)(() => {
      const state = gameState.current;
      ...
      state.gates.forEach((g) => {
        ...
        const drawBox = (x, w, power) => {
          ...
          ctx.lineWidth = 3;
          ctx.beginPath();
          if (ctx.roundRect) ctx.roundRect(x, g.y, w, 40, 8);
          else ctx.rect(x, g.y, w, 40);
          ctx.fill();
          ctx.stroke();
          ctx.fillStyle = "#fff";
      ...
      if (!state.ship.isDead) {
        ...
        ctx.globalAlpha = isVictory ? 0.9 : 0.5 + Math.random() * 0.5;
        const tLen = isVictory ? 80 : 35;
        ctx.beginPath();
        ctx.moveTo(-12, 20);
        ctx.lineTo(12, 20);
        ctx.lineTo(0, tLen);
        ctx.fill();
        ctx.globalAlpha = 1;
        ctx.fillStyle = isVictory ? "#aaffff" : "#00ffcc";
        if (isVictory) {
      ...
      if (!state.ship.isDead) {
        const isVictory = gameResult === "success" && gamePhase === "ended";
        ctx.fillStyle = isVictory ? "#00ffff" : "#ffffff";
      ...
      ctx.restore();
      if (state.flash > 0) {
        const flashColor = gameResult === "success" ? "200, 255, 255" : "255, 255, 255";
      ...
      if (state.vignette !== 0) {
        ...
        if (state.vignette > 0) {
          ...
          grad.addColorStop(1, `rgba(100, 0, 10, ${state.vignette * 0.85})`);
        } else {
          grad.addColorStop(0, `rgba(0, 255, 255, ${Math.abs(state.vignette) * 0.3})`);
      ...
      if (state.resultAnim.type) {
        ...
        const isSuccess = s.type !== "FAILURE";
        const bounce = isSuccess ? Math.sin(progress * Math.PI) * 0.2 : 0;
        ...
        ctx.shadowBlur = isSuccess ? 70 : 35;
        ...
        if (state.failReason) {
          ...
          ctx.shadowBlur = 0;
          ctx.fillText(state.failReason, 0, 100);
          if (isSuccess) {
            ctx.font = "bold 22px monospace";
            ctx.fillStyle = "#00ffff";
            ctx.fillText("\u25B6 \uACB0\uACFC\uB97C \uC131\uACF5\uC801\uC73C\uB85C \uBC14\uAFE8\uC2B5\uB2C8\uB2E4", 0, 145);
          } else {
            ctx.font = "18px monospace";
            ctx.fillStyle = "#94a3b8";
            ctx.fillText("\uB354 \uB098\uC740 \uC120\uD0DD\uC774 \uAC00\uB2A5\uD588\uC2B5\uB2C8\uB2E4", 0, 145);
          }
        }
        ctx.restore();
      }
      if (gamePhase === "rewind_watch" || gamePhase === "rewind_back") {
        ...
        ctx.textAlign = "center";
        ctx.shadowColor = "rgba(255, 50, 80, 0.8)";
        ctx.shadowBlur = 20;
        ctx.fillText(gamePhase === "rewind_watch" ? "COLLISION!" : "\u25C0\u25C0 REWIND", width / 2, height / 2 + 80);
        ctx.shadowBlur = 0;
      }
      if (gamePhase === "ended") {
        ctx.fillStyle = gameResult === "success" ? "rgba(0, 40, 30, 0.15)" : "rgba(0, 0, 0, 0.45)";
        ctx.fillRect(0, 0, width, height);
      }
    }, [gamePhase, gameResult, endStimulus]);
    (0, import_react.useEffect)(() => {
      window.AD_PHASE = gamePhase;
    }, [gamePhase]);
    (0, import_react.useEffect)(() => {
      window.AD_STATE = gameState.current;
    }, [stage]);
    (0, import_react.useEffect)(() => {
    ...
    return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "microgate-app", children: [
      stage === STAGES.PLAY && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "game-container", children: [
        CFG.debug && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", { className: "dbg-line dbg-overlay", children: [
          "sid=",
          CFG.sid,
          " \xB7 mode=",
          CFG.mode,
          ...
          " \xB7 ",
          gamePhase,
          LOG.VER_FALLBACK ? " \xB7 \u26A0 ver=B \uC5C6\uC74C \u2192 A \uC7AC\uC0DD" : ""
        ] }),
        IS_TUTORIAL && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "tutorial-overlay", style: { position: "absolute", top: "20px", left: "50%", transform: "translateX(-50%)", background: "rgba(0,0,0,0.7)", padding: "10px 20px", borderRadius: "20px", zIndex: 10, pointerEvents: "none", width: "auto", textAlign: "center", color: "#00ffcc", border: "1px solid #00ffcc" }, children: "\uB9C8\uC6B0\uC2A4/\uB4DC\uB798\uADF8\uB85C \uC88C\uC6B0\uB85C \uC774\uB3D9\uD558\uC5EC \uD30C\uC6CC\uB97C \uB192\uC774\uC138\uC694!" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("canvas", { ref: canvasRef, width: "480", height: "720", className: "game-canvas" })
      ] }),
      stage === STAGES.ENDED && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "card completion-card", children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { style: { textAlign: "center" }, children: IS_TUTORIAL ? "\uC5F0\uC2B5\uC774 \uB05D\uB0AC\uC2B5\uB2C8\uB2E4." : "\uC774 \uAD11\uACE0\uAC00 \uB05D\uB0AC\uC2B5\uB2C8\uB2E4." }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { className: "dbg-line", style: { textAlign: "center" }, children: "\uC7A0\uC2DC\uB9CC \uAE30\uB2E4\uB824 \uC8FC\uC138\uC694\u2026" }),
        CFG.debug && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("pre", { className: "dbg-line", children: JSON.stringify(LOG, null, 1) })
      ] })
    ] });
  };
  var MicrogateExperiment_default = MicrogateExperiment;

  // src/App.jsx
  var import_jsx_runtime2 = __toESM(require_jsx_runtime(), 1);
  function App() {
    return /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "App", children: /* @__PURE__ */ (0, import_jsx_runtime2.jsx)(MicrogateExperiment_default, {}) });
  }
  var App_default = App;
  ...
  import_client.default.createRoot(document.getElementById("root")).render(
    /* @__PURE__ */ (0, import_jsx_runtime3.jsx)(import_react3.default.StrictMode, { children: /* @__PURE__ */ (0, import_jsx_runtime3.jsx)(App_default, {}) })
  );
})();
