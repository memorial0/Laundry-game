...
CLIPS = {
    # 1·4·11 은 한때 ver 로 갈렸다(s1A 셔츠 / s1B 수건). 소재를 셔츠+청바지로
    # 통일하면서(2026-09-11) 판별자를 뗐다 — 두 버전이 같은 클립을 듣는다.
    's1':  ('내일 입을 셔츠, 오늘 같이 빨래하기', None),
    's2':  ('그대로 세탁 시작', None),
    's3':  ('세탁 중…', '세탁 중.'),
    's4':  ('셔츠 색이 변해 버렸다', None),
    # 장면 11 은 장면 1("내일 입을 셔츠…")을 과거로 받는다.
    's11': ('내일 입으려던 셔츠였다', None),
...
def synth(text, rate, wav):
    ...
    문장 세 번이 180.7 / 196.9 / 204.2Hz 였다(2.1반음 폭). 그래서 (소재가 갈렸던
    시절의) ver A 와 ver B 처럼 낱말 하나만 다른 짝이 억양까지 갈렸다. 실제로 첫
    판에서 장면 4가 A 232Hz · B 182Hz 로 4반음 벌어져 있었다 — 이 광고의 전환점
    대사인데 버전마다 다른 무게로 들렸다는 뜻이다. 지금 남은 짝은 s6i/s6w 다.
...
WAV_ORDER = ['s1', 's2', 's3', 's4', 's11', 's5', 's6i', 's7', 's8', 's9', 's10']
...
def main():
    ...
    body = [
        ...
        '   *  voice.test.js 가 이 값으로 (a) mode 로 갈리는 짝(s6i/s6w)이 같은 처리를 받았는지',
